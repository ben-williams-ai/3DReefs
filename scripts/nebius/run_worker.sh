...
PRODUCTION_CANARY="${PRODUCTION_CANARY:-false}"
PRODUCTION_RESUME_LOCAL="${PRODUCTION_RESUME_LOCAL:-false}"
PRODUCTION_RETRAIN_FAILED="${PRODUCTION_RETRAIN_FAILED:-false}"
PRODUCTION_PATCH_IDS="${PRODUCTION_PATCH_IDS:-null}"
...
upload_outputs() {
  ...
  if [[ -d "${OUT_ROOT}/project/runs/${RUN_ID}" ]]; then
    local sync_args=(--no-follow-symlinks)
    if [[ "${WORKER_MODE}" == "production" ]]; then
      sync_args+=(
        --exclude "colour_restoration/outputs/*/images/*"
        --exclude "splat/patches/*/selected_images/*"
        --exclude "sfm/undistorted/images/*"
        --exclude "sfm/undistorted_2048/images/*"
      )
    fi
    aws_s3 sync "${OUT_ROOT}/project/runs/${RUN_ID}" "s3://${BUCKET}/${OUTPUT_PREFIX}/runs/${RUN_ID}/" \
      "${sync_args[@]}" || upload_code=1
  fi
...
PY
  ...
  if [[ "${WORKER_MODE}" == "production" && "${code}" -eq 0 && "${upload_code}" -eq 0 ]]; then
    verify_production_upload || upload_code=1
  fi
...
verify_production_upload() {
  local run_dir="${OUT_ROOT}/project/runs/${RUN_ID}"
  local destination="s3://${BUCKET}/${OUTPUT_PREFIX}/runs/${RUN_ID}"
...
patches = []
for metadata_path in sorted((run_dir / "splat" / "patches").glob("*/patch_metadata.json")):
    metadata = json.loads(metadata_path.read_text(encoding="utf-8"))
    patch_id = metadata_path.parent.name
    record = {
        "patch_id": patch_id,
        "layout_status": metadata.get("status"),
        "invalid_reasons": metadata.get("invalid_reasons", []),
    }
    status_path = metadata_path.parent / "splat" / "training_status.json"
    if status_path.is_file():
        status = json.loads(status_path.read_text(encoding="utf-8"))
        record.update({
            "training_status": status.get("status"),
            "terminal_reason": status.get("reason"),
            "completed_iterations": status.get("completed_iterations"),
            "completion_ratio": status.get("completion_ratio"),
            "reported_splat_count": status.get("final_splat_count"),
        })
        output_value = status.get("output_file")
        if output_value:
            output = Path(str(output_value))
            container_run = Path("/scratch/3dreefs/project/runs") / run_dir.name
            if not output.is_file() and output.is_relative_to(container_run):
                output = run_dir / output.relative_to(container_run)
            if output.is_file():
                ...
                with output.open("rb") as handle:
                    for line in handle:
                        ...
                        if line.strip() == b"end_header":
                            break
                record.update({
                    "output_file": str(output.relative_to(run_dir)),
                    "output_size_bytes": output.stat().st_size,
                    "output_sha256": digest.hexdigest(),
                    "ply_vertex_count": count,
                })
    patches.append(record)
valid = [item for item in patches if item["layout_status"] == "valid"]
accepted = [
    item for item in valid
    if float(item.get("completion_ratio") or 0) >= 0.85
    and item.get("ply_vertex_count") == 2_000_000
]
terminal = [
    item for item in valid
    if item.get("training_status") in {"complete", "warning", "severe_warning", "failed"}
]
if len(terminal) != len(valid):
    raise SystemExit("not every valid production patch has a terminal training status")
payload = {
    "status": "terminal",
    "valid_patch_count": len(valid),
    "accepted_patch_count": len(accepted),
    "scientific_failure_count": len(valid) - len(accepted),
    "below_30000_iterations": [
        item["patch_id"] for item in valid
        if int(item.get("completed_iterations") or 0) < 30000
    ],
    "patches": patches,
}
...
PY
  aws_s3 cp "${run_dir}/production_manifest.json" "${destination}/production_manifest.json"
  while IFS=$'\t' read -r relative_path expected_hash; do
    local remote_hash
    remote_hash="$(aws_s3 cp "${destination}/${relative_path}" - | sha256sum | awk '{print $1}')"
    [[ "${remote_hash}" == "${expected_hash}" ]] || {
      echo "Production PLY read-back hash mismatch: ${relative_path}" >&2
      return 1
    }
  done < <(python3 - "${run_dir}/production_manifest.json" <<'PY'
import json
import sys

for patch in json.load(open(sys.argv[1], encoding="utf-8"))["patches"]:
    if patch.get("output_file"):
        print(f'{patch["output_file"]}\t{patch["output_sha256"]}')
PY
  )
  printf '{"status":"verified_complete","pipeline_exit":0,"upload_status":0}\n' \
    > "${run_dir}/production_complete.json"
  aws_s3 cp "${run_dir}/production_complete.json" "${destination}/production_complete.json"
  [[ "$(aws_s3 cp "${destination}/production_complete.json" -)" == \
    "$(tr -d '\n' < "${run_dir}/production_complete.json")" ]]
}
...
upload_production_canary() {
  local run_dir="${OUT_ROOT}/project/runs/${RUN_ID}"
  local marker="${run_dir}/canary_accepted.json"
  local acknowledgement="${OUT_ROOT}/canary_upload_complete"
  [[ -f "${marker}" && ! -f "${acknowledgement}" ]] || return 0
  local relative_output output_file local_hash remote_hash destination
  relative_output="$(python3 -c 'import json,sys; print(json.load(open(sys.argv[1]))["output_file"])' "${marker}")"
  output_file="${run_dir}/${relative_output}"
  test -f "${output_file}"
  local_hash="$(sha256sum "${output_file}" | awk '{print $1}')"
  destination="s3://${BUCKET}/${OUTPUT_PREFIX}/runs/${RUN_ID}/canary"
  aws_s3 cp "${output_file}" "${destination}/$(basename "${output_file}")"
  aws_s3 cp "${marker}" "${destination}/canary_accepted.json"
  aws_s3 cp "$(dirname "${output_file}")/training_status.json" "${destination}/training_status.json"
  remote_hash="$(aws_s3 cp "${destination}/$(basename "${output_file}")" - | sha256sum | awk '{print $1}')"
  [[ "${remote_hash}" == "${local_hash}" ]] || {
    echo "Canary PLY read-back hash mismatch." >&2
    return 1
  }
  printf '{"status":"verified","ply_sha256":"%s"}\n' "${local_hash}" > "${run_dir}/canary_verified.json"
  aws_s3 cp "${run_dir}/canary_verified.json" "${destination}/canary_verified.json"
  [[ "$(aws_s3 cp "${destination}/canary_verified.json" -)" == \
    "$(tr -d '\n' < "${run_dir}/canary_verified.json")" ]]
...
if [[ "${WORKER_MODE}" == "production" && "${PRODUCTION_RESUME_LOCAL}" == "true" ]]; then
  test -d "${OUT_ROOT}/project/runs/${RUN_ID}/sfm/undistorted/images"
  test -d "${OUT_ROOT}/project/runs/${RUN_ID}/colour_restoration/outputs/undistorted/images"
  test -d "${OUT_ROOT}/project/runs/${RUN_ID}/splat/patches"
  test -L "${DATASET_DIR}/raw_images"
elif [[ "${WORKER_MODE}" == "stage2_splat_eval" || "${WORKER_MODE}" == "production" ]]; then
  require_env SOURCE_BUNDLE_URI
  if [[ -L "${DATASET_DIR}/raw_images" ]]; then
    rm "${DATASET_DIR}/raw_images"
  fi
  mkdir -p "${DATASET_DIR}/raw_images"
  SOURCE_LOCAL_RUN_ID="sfm_${DATASET_NAME}_${SOURCE_VARIANT}"
  if [[ "${WORKER_MODE}" == "production" ]]; then
    SOURCE_LOCAL_RUN_ID="${RUN_ID}"
    TRAINING_WORKSPACE="undistorted_2048"
  else
  ...
  if [[ "${WORKER_MODE}" == "stage2_splat_eval" ]]; then
    source_sync_args+=(--include "sfm/undistorted_full_resolution/*")
  else
    source_sync_args+=(--only-show-errors)
  fi
  ...
  if [[ "${WORKER_MODE}" == "production" ]]; then
    (
      cd "${OUT_ROOT}/project/runs/${SOURCE_LOCAL_RUN_ID}"
      awk '$2 == "worker_identity.json" || $2 == "effective_config.yml" || $2 == "run_manifest.json" || $2 == "sfm/image_mapping.json" || index($2, "sfm/undistorted_2048/") == 1 {print}' \
        checksums.sha256 | sha256sum --check --strict --quiet
    )
    mv "${OUT_ROOT}/project/runs/${SOURCE_LOCAL_RUN_ID}/sfm/undistorted_2048" \
      "${OUT_ROOT}/project/runs/${SOURCE_LOCAL_RUN_ID}/sfm/undistorted"
  fi
  rmdir "${DATASET_DIR}/raw_images"
  if [[ "${WORKER_MODE}" == "production" ]]; then
    TRAINING_WORKSPACE="undistorted"
  fi
...
if [[ "${WORKER_MODE}" != "stage2_splat_eval" && "${WORKER_MODE}" != "production" && -z "${VOCAB_TREE_S3_URI:-}" ]]; then
...
docker_args=(
  ...
  -e PRODUCTION_CANARY="${PRODUCTION_CANARY}" \
  -e PRODUCTION_RESUME_LOCAL="${PRODUCTION_RESUME_LOCAL}" \
  -e PRODUCTION_RETRAIN_FAILED="${PRODUCTION_RETRAIN_FAILED}" \
  -e PRODUCTION_PATCH_IDS="${PRODUCTION_PATCH_IDS}" \
...
if [[ "${WORKER_MODE}" == "production" ]]; then
  ...
  "${REEFS_VENV}/bin/python" - "${CONFIG_PATH}" "${PROFILE_CONFIG}" "${DATASET_NAME}" "${PWD}" <<'PY'
import sys
from pathlib import Path
import yaml

source, destination, dataset_name, repo_root = sys.argv[1:]
profiles = {
    "dataset1": "dataset1-colour.json",
    "dataset2": "dataset_02-colour.json",
    "dataset3": "dataset_03-colour.json",
    "dataset4": "dataset_04-colour.json",
    "dataset5": "dataset_05-colour.json",
    "dataset6": "dataset_06-colour.json",
}
profile = Path(repo_root) / "configs" / "colour-profiles" / profiles[dataset_name]
...
elif [[ -f /job/colour_profile.json ]]; then
  PROFILE_CONFIG=/scratch/3dreefs/profile-config.yml
...
run_production() {
  local run_dir="/scratch/3dreefs/project/runs/${RUN_ID}"
  local production_args=(
    --advanced.eval.enabled false
    --advanced.splat.patching.max_cameras 200
    --advanced.splat.patching.patch_ids null
    --advanced.splat.train.num_iters 30000
    --advanced.splat.train.num_splats_per_patch 2000000
    --advanced.splat.train.strategy mcmc
    --advanced.splat.train.headless true
    --advanced.splat.train.max_width 2048
    --advanced.splat.train.retry_max_width "[]"
    --advanced.splat.train.retrain_failed "${PRODUCTION_RETRAIN_FAILED}"
  )
  "${REEFS_VENV}/bin/python" - "${run_dir}/source_manifest.json" "${DATASET_NAME}" <<'PY'
import json
import sys
from pathlib import Path
...
if manifest.get("status") != "validated":
    raise SystemExit("source manifest is not validated")
if manifest.get("dataset") != sys.argv[2]:
    raise SystemExit("source manifest dataset mismatch")
if manifest.get("source_variant") != "sfm_1024_sift_global":
    raise SystemExit("source manifest has the wrong SfM variant")
workspace = manifest.get("workspaces", {}).get("2048", {})
if workspace.get("image_file_count", 0) <= 0:
    raise SystemExit("source manifest has no 2048 images")
if workspace.get("registered_image_count") != workspace.get("image_file_count"):
    raise SystemExit("2048 image and registered-image counts differ")
PY
  if [[ "${PRODUCTION_RESUME_LOCAL}" != "true" ]]; then
    run_pipeline "splat.patch" "overwrite" "${production_args[@]}"
  fi
  local valid_patches first_patch remaining_patches
  valid_patches="$(
    "${REEFS_VENV}/bin/python" - "${run_dir}/splat/patches" <<'PY'
import json
import sys
from pathlib import Path
...
valid = []
for metadata_path in sorted(patches.glob("*/patch_metadata.json")):
    metadata = json.loads(metadata_path.read_text(encoding="utf-8"))
    if metadata.get("status") == "valid":
        valid.append(metadata_path.parent.name)
if not valid:
    raise SystemExit("production layout contains no valid patches")
print(",".join(valid))
PY
  )"
  first_patch="${valid_patches%%,*}"
  if [[ "${PRODUCTION_CANARY}" == "true" ]]; then
    run_pipeline "splat.train" "overwrite" "${production_args[@]}" \
      --advanced.splat.train.patch_ids "[${first_patch}]"
    "${REEFS_VENV}/bin/python" - "${run_dir}" "${first_patch}" <<'PY'
import json
import sys
from pathlib import Path

from reefs.postprocess.artifacts import ply_vertex_count

run_dir, patch_id = Path(sys.argv[1]), sys.argv[2]
status_path = run_dir / "splat" / "patches" / patch_id / "splat" / "training_status.json"
status = json.loads(status_path.read_text(encoding="utf-8"))
output = Path(str(status.get("output_file", "")))
if status.get("status") != "complete":
    raise SystemExit(f"canary status is not complete: {status}")
if status.get("completed_iterations") != 30000:
    raise SystemExit("canary did not reach iteration 30000")
if status.get("final_splat_count") != 2000000:
    raise SystemExit("canary log did not report exactly 2000000 splats")
if not output.is_file() or ply_vertex_count(output) != 2000000:
    raise SystemExit("canary PLY does not contain exactly 2000000 vertices")
marker = {
    "status": "accepted",
    "patch_id": patch_id,
    "completed_iterations": 30000,
    "splat_count": 2000000,
    "output_file": str(output.relative_to(run_dir)),
}
(run_dir / "canary_accepted.json").write_text(json.dumps(marker, indent=2) + "\n", encoding="utf-8")
PY
  fi
  run_pipeline "splat.train" "resume" "${production_args[@]}" \
    --advanced.splat.train.patch_ids "${PRODUCTION_PATCH_IDS}"
}
...
if [[ "${WORKER_MODE}" == "stage2_source" || "${WORKER_MODE}" == "stage2_source_recovery" ]]; then
  ...
  run_stage2_batch
elif [[ "${WORKER_MODE}" == "production" ]]; then
  run_production
...
if [[ "${WORKER_MODE}" == "stage2_splat_eval" ]]; then
  ...
  upload_completed_stage2_probes
elif [[ "${WORKER_MODE}" == "production" && "${PRODUCTION_CANARY}" == "true" ]]; then
  while kill -0 "${docker_pid}" >/dev/null 2>&1; do
    upload_production_canary
    sleep 30
  done

...
: "${DATASET_NAME:?Set DATASET_NAME, e.g. dataset7}"
: "${RESUME_FROM_S3_URI:?Set RESUME_FROM_S3_URI to the durable failed source prefix}"
: "${RUN_ID:?Set a new, empty retry RUN_ID}"
...
export WORKER_MODE="stage2_source_recovery"
...
aws s3 ls "${RESUME_FROM_S3_URI%/}/sfm/database.db" \
  --endpoint-url "${ENDPOINT_URL:-https://storage.eu-north1.nebius.cloud}" >/dev/null
aws s3 ls "${RESUME_FROM_S3_URI%/}/sfm/selected_sparse/" \
  --endpoint-url "${ENDPOINT_URL:-https://storage.eu-north1.nebius.cloud}" | grep -q .
if aws s3 ls "s3://${BUCKET}/${OUTPUT_PREFIX}/runs/${RUN_ID}/" \
  ...
  echo "Refusing to reuse non-empty Stage 2 recovery prefix for ${RUN_ID}." >&2
  exit 2
fi

printf 'Stage 2 source recovery: dataset=%s run=%s source=%s git=%s\n' \
  "${DATASET_NAME}" "${RUN_ID}" "${RESUME_FROM_S3_URI}" "${GIT_REF}"

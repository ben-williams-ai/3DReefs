...
for name in BUCKET INPUT_PREFIX OUTPUT_PREFIX IMAGE_NAME GIT_REPO GIT_REF DATASET_NAME RUN_ID CONFIG_IN_REPO STEPS RESUME_POLICY EXTRA_ARGS VOCAB_TREE_S3_URI; do
  if [[ -n "${!name:-}" ]]; then
    printf '%s=%q\n' "${name}" "${!name}" >> "${ENV_FILE}"
  fi
done

...
- **FR-006**: The system MUST create patch extents from the reconstruction using `wildflow.splat.patches` birds-eye spatial regions only as anchors, then assign cameras using the single supported camera-selection approach defined by the active camera-selection feature.

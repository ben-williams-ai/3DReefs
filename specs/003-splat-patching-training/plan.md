...
**Performance Goals**: Fail missing or incompatible SfM/patch/training inputs before expensive LFS work; train exactly one patch at a time; record timings for outlier filtering, patch generation, and every requested patch training job.  
...
- **External Tool Validation**: PASS. LFS is validated before training; Feature 1
  tool path resolution is reused; pycolmap dependency is tested at import time
  before patching.
...
- Treat large proposed camera removals as ambiguous, not ordinary outlier
  removal.
- Generate patch anchors from camera-position birds-eye regions, then perform
  final camera assignment with view-based scoring.
...
- **Modular, Testable Implementation**: PASS. Planned modules isolate config,
  ...
  run-record updates.
- **External Tool Validation**: PASS. LFS and pycolmap availability are checked
  before heavy work; LFS failure modes are captured per patch.

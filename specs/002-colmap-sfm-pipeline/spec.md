...
8. **Given** recoloured images are enabled, **When** preflight runs, **Then** the system verifies that recoloured images mirror raw image relative paths, filenames, and dimensions for later splatting-stage image use; SfM and COLMAP undistortion still use raw images.
...
### User Story 4 - Use Recoloured Images Only For Splatting Inputs (Priority: P4)
...
**Independent Test**: Can be tested with a project where raw and recoloured images have matching layouts and filenames. SfM and COLMAP undistortion should use raw images, then splatting-stage image selection should use recoloured images only when the recoloured-image setting is enabled and validated.

**Acceptance Scenarios**:

1. **Given** recoloured images are disabled, **When** undistortion runs, **Then** the system undistorts raw images and downstream splatting uses raw-image inputs.
2. **Given** recoloured images are enabled and valid, **When** undistortion runs and downstream splatting inputs are selected, **Then** COLMAP undistortion still uses raw images and splatting-stage image inputs use matching recoloured images.
...
- **FR-012**: The system MUST ensure recoloured images are not used for feature extraction, matching, sparse reconstruction, or COLMAP undistortion.
- **FR-013**: The system MUST use recoloured images only as splatting-stage image inputs when recoloured-image use is enabled and validation passes.
- **FR-014**: The system MUST use raw images for COLMAP undistortion in every recoloured-image mode.

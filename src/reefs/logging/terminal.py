...
class TerminalReporter:
    """Mirror important pipeline events to the terminal."""
    ...
    def info(self, message: str) -> None:
        """Print and optionally persist an informational message."""
        print(message, file=self.stream, flush=True)
        if self.logger:
            self.logger.info(message)

    def warning(self, message: str) -> None:
        """Print and optionally persist a warning message."""
        print(message, file=self.stream, flush=True)
        if self.logger:
    ...
    def tee_line(self, line: str) -> None:
        """Print one external-tool output line exactly once."""

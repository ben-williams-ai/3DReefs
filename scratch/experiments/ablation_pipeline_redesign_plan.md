...
- Formal ablation comparisons should treat COLMAP and LFS commits as controlled inputs, not incidental runtime details.
- The intended COLMAP baseline is COLMAP `4.0.4`, commit `9c23f694`, with CUDA/cuDSS. Earlier successful local and archived reef runs used this build.
- The intended LFS baseline for large reef MCMC runs is LichtFeld Studio commit `6d591a34`, reported by logs as `v0.5.2-380-g6d591a34`.
- Why these pins exist:
  - COLMAP `4.0.4` commit `9c23f694` is the validated SfM baseline for earlier reef runs and the build used when dense/mesh behaviour was checked; changing it changes reconstruction behaviour and breaks comparability.
  - LFS commit `6d591a34` was selected after `v0.5.2` hit full-width FastGS bucket-buffer OOMs on known reef MCMC patches; the pinned commit completed those patches at 30,000 iterations and 1,000,000 splats.
- Do not change COLMAP or LFS versions/commits for formal sweep jobs unless the change is deliberate, recorded in the run identity/manifests, and validated with the small GPU bundle-adjustment smoke plus one end-to-end smoke.
- Historical note: Docker/Nebius temporarily drifted to COLMAP commit `5f35f398`, which reports as `COLMAP 3.14.0.dev0`; results from that build should not be compared to `4.0.4` results without explicitly marking the toolchain difference.
...
- Cross-camera pair generation and the explicit COLMAP `matches_importer` pass are part of the AIMS baseline and must stay enabled unless a variant is explicitly sweeping or disabling paired-camera matching.
- Stage 1 `*_global` names refer to the reconstruction backend (`colmap global_mapper`, the COLMAP 4 integrated GLOMAP-style global SfM mapper), not to the matcher. The intended Stage 1 baseline is sequential matching + vocab-tree loop detection + explicit paired-camera matching, followed by `global_mapper` or `mapper` depending on the mapper sweep dimension.
- Patch cap default: 400 cameras.
- Splat cap default: 1,000,000 unless changed by Stage 2.
...
- [x] Correct ablation baseline drift: `advanced.sfm.matching.cross_camera_pairs.run_matching_pass` must be `true`, matching `configs/example.yml` and the dataset configs. The earlier `false` setting was a mistake, not an intentional sweep simplification.
...
2.5. Formal undistorted held-out eval smoke:
   - [x] **NEW** after the local held-out eval proof passes, run at most one cheap Nebius smoke proving the same target-source labelling if Docker or worker behaviour changed.
...
- 2026-07-04 resumed-eval hygiene fix: the first LPIPS Nebius smoke on Git ref `93878bd` produced real LPIPS and the correct `training_undistorted` target label, but it also exposed stale metrics contamination from the restored run tree: `attempt_1/metrics.csv` contained an old `500` row plus the new `500` row. Fix committed and pushed as `badb950 Keep resumed eval metrics isolated`: main `splat.eval` now writes to the next fresh `attempt_N` directory when prior eval attempts exist, and the Nebius `EVAL_PATCH_COUNT` path uses `overwrite` for selected `splat.train,splat.eval` outputs while preserving restored SfM/patch inputs.
- 2026-07-04 resumed-eval verification: `uv run pytest tests/integration/test_splat_mocked_success.py::test_splat_eval_resume_uses_fresh_attempt_dir tests/integration/test_splat_mocked_success.py::test_splat_eval_writes_eval_manifests_and_metrics tests/unit/test_lfs_commands.py tests/unit/test_eval_lpips.py` passed, 9 tests. `bash -n scripts/nebius/run_ablation_worker.sh && git diff --check` also passed before commit.
- 2026-07-04 clean LPIPS Nebius smoke passed after the hygiene fix: `nebius_lpips_eval_clean_test_dataset_badb950_20260704T104729Z` uploaded `EXIT:0` under `s3://3dreefs-ben-eu-north1/experiments/ablations/smoke/runs/nebius_lpips_eval_clean_test_dataset_badb950_20260704T104729Z/`, used image `cr.eu-north1.nebius.cloud/e00eqkjz0mkvvedmrd/3dreefs:ac6ffaa`, digest `sha256:faac5261bef90d4d9e97abc2173c31c510820e86dc6eb8240453599e61a05e3b`, Git ref/commit `badb950`, and config `configs/docker-test.yml`. The VM deleted after upload; `nebius compute instance list` showed only the separate dataset4 pilot VM afterwards.
- 2026-07-04 clean LPIPS Nebius smoke metrics: `run_status.json` is complete with no warnings/errors; `metrics_long.csv` has a single fresh row, `p001,attempt_1,500,10.54285,0.338179,0.929121,training_undistorted,4096,3602,1.899174,18348`; `metrics_final.csv` matches that final row; `lpips_metrics.json` records `lpips.LPIPS(net='alex')` with value `0.9291208386421204`; `eval_step_500/*.png` contains the saved held-out comparison images used for LPIPS. The smoke still re-uploaded about 7.5 GiB because the restored source tree is synced wholesale, so upload scope remains a cost/performance caveat for repeated smoke jobs.
- 2026-07-04 representative dataset4 pilot terminal check: `pilot_dataset4_sfm_1024_sift_global_ef07b36_20260704T015518Z` uploaded `EXIT:1` under `s3://3dreefs-ben-eu-north1/experiments/ablations/pilot/runs/pilot_dataset4_sfm_1024_sift_global_ef07b36_20260704T015518Z/` and the VM became unreachable after upload, consistent with deletion. SfM itself completed and the patch command exited 0, creating 34 patches and selecting `p000;p004;p007;p011;p015;p018;p022;p026;p029;p033`, but `results_splat.csv` stayed empty and the eval summary left all 10 rows pending. Treat this old-image pilot as diagnostic only until the post-patch validation-splat failure path is understood.
- 2026-07-04 dataset2 1024 Stage 1 probe live note: `stage1_probe_dataset2_sfm_1024_sift_global_d4d7899_20260704T113722Z` is intentionally running on dataset2 with image `cr.eu-north1.nebius.cloud/e00eqkjz0mkvvedmrd/3dreefs:ac6ffaa` and Git ref `d4d7899`. Main extraction completed all 6,590 images in 8,375.21s and sequential matching with loop detection completed in 1,733.64s. The global mapper has been CPU-active but silent in `=== Running iterative retriangulation and refinement ===` for several hours; no sparse files have been written yet, `run_status.json` still has no warnings/errors, and memory/disk remain healthy. This may make full dataset2/global/1024 unsuitable as the first cheap formal probe if it continues this long, even though it has not failed.
- 2026-07-05 dataset2 1024 Stage 1 probe continuation: the initial global mapper completed successfully after 28,903.26s and selected sparse text export completed after 69.96s. Sparse refinement repeat 1 then completed point triangulation in 1,611.68s and point filtering in 22.83s. As of 02:57 UTC, `sfm.refine.iter_01.bundle_adjuster` is still CPU-active after about 3h43m with no output files yet under `refined_sparse/iter_01/bundle_adjusted`, no recorded warnings/errors, and healthy memory/disk. This reinforces that the formal sweep should avoid large full-dataset global SfM probes until the intended runtime budget is explicit.
- 2026-07-05 dataset2 1024 Stage 1 probe terminal outcome: `stage1_probe_dataset2_sfm_1024_sift_global_d4d7899_20260704T113722Z` uploaded `EXIT:0` and deleted the VM, but the experiment row is `status=failed` with `failure_reason=sfm_timeout_exceeded_20h`. The timeout occurred during `sfm.refine.iter_02.bundle_adjuster`; prior completed stages included repeat 1 bundle adjustment in 16,673.97s, repeat 2 triangulation in 1,578.16s, and repeat 2 filtering in 22.67s. No patching, splatting, or eval ran. Uploaded artefacts are the ablation ledgers/configs/identity/events/resource samples and the exit marker only, not the large partial SfM project tree. This is a useful negative probe: dataset2 + global mapper + two sparse-refinement repeats is too large for the current 20h Stage 1 budget.
- 2026-07-05 dataset2 timeout forensics: older successful dataset2 global runs under `data/dataset2/runs` and the 2026-06-23 archive used COLMAP `4.0.4` commit `9c23f694`, mostly no SIFT affine shape, no DSP, sequential+full-vocab matching, and no post-reconstruction `sfm.refine.iter_*` loop. The new dataset2 1024 probe used COLMAP commit `5f35f398` from the Docker/Nebius path, SIFT affine+DSP, sequential loop-only matching with overlap 30/loop NN 5, and sparse refinement repeats 2. Reconstructing the command from the uploaded effective config shows main `global_mapper` had `--GlobalMapper.gp_use_gpu 1 --GlobalMapper.ba_ceres_use_gpu 1`, but sparse-refinement `bundle_adjuster` emitted no GPU/refine flags because `advanced.sfm.sparse_refinement.bundle_adjuster.use_gpu` and refine switches were `null`. Treat the immediate timeout cause as the full-dataset sparse-refinement loop, with likely config/code bug that refinement does not inherit reconstruction GPU BA settings. Scratch evidence/report: `scratch/experiments/dataset2_timeout_forensics/README.md`.
- 2026-07-05 correction: the first canonical dataset1/dataset2 Stage 1 probe launch was aborted because `experiments/ablations/ablation_config.yml` incorrectly set `advanced.sfm.matching.cross_camera_pairs.run_matching_pass: false`. That meant paired-camera candidates were generated but not imported/matched into COLMAP. This was wrong for the AIMS baseline. The canonical prefixes were deleted and the Nebius VMs were removed. Relaunch only after verifying the effective config says `advanced.sfm.matching.cross_camera_pairs.run_matching_pass=true`.
- 2026-07-05 sparse-refinement fix: `build_sparse_refinement_iteration_commands()` now inherits `reconstruction.use_gpu` and intrinsics-refine switches when sparse bundle-adjuster overrides are `null`, while still allowing explicit sparse-refinement overrides. Rebuilding the failed dataset2 effective config now emits `--BundleAdjustment.refine_focal_length 1 --BundleAdjustment.refine_principal_point 1 --BundleAdjustment.refine_extra_params 1 --BundleAdjustmentCeres.use_gpu 1`. Verification: `uv run pytest tests/unit/test_colmap_commands.py -q` passed, `bash -n scripts/nebius/run_ablation_worker.sh && git diff --check` passed, and a watched Docker smoke ran `/opt/colmap/bin/colmap bundle_adjuster` on the existing small Docker-smoke selected sparse model with those GPU/refine flags. The smoke converged in 4.92s, wrote binary sparse outputs under `scratch/experiments/sparse_refine_gpu_smoke_20260705/out`, and sampled GPU memory rising from about 10.3 GiB to 11.1 GiB. Important caveat: this validates the flag path on a small model only; it does not prove full dataset2 two-repeat refinement is a sensible sweep default.
- 2026-07-05 timeout diagnostics fix: `scripts/nebius/run_ablation_worker.sh` now uploads lightweight diagnostics from every actual pipeline run directory under `/project/runs/*` to `.../diagnostics/runs/<run_id>/`, including `run_status.json`, `timings.json`, effective config, CLI overrides, run manifest, resource summaries, and pipeline/COLMAP/warnings logs when present. This avoids the Stage 1 outer-run-id mismatch that hid the actual `sfm_dataset2_*` run logs on timeout, without syncing huge partial sparse/image trees by default.
- 2026-07-05 toolchain pin audit: README history shows the original setup notes introduced COLMAP `4.0.4` and LFS `v0.5.2` on 2026-05-28, then LFS was explicitly moved to upstream commit `6d591a34` on 2026-06-17 to fix full-width FastGS bucket-buffer OOMs on known reef patches. Docker initially used COLMAP `4.0.4` on 2026-06-24/25, then commit `d91e8b3` on 2026-06-26 changed Docker to COLMAP commit `5f35f398`. Treat `5f35f398` as a toolchain drift until Docker/Nebius is realigned or the drift is deliberately accepted as a new baseline.
- 2026-07-05 toolchain rule: formal ablation jobs should use COLMAP `4.0.4` commit `9c23f694` with CUDA/cuDSS and LFS commit `6d591a34` unless a toolchain change is explicitly proposed, documented, and revalidated. Any COLMAP/LFS version or commit change must be recorded in run identity, effective config, manifests, ledgers, and README/workflow notes.

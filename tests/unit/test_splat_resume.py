...
def test_discover_existing_splat_outputs_respects_requested_training_patch_ids(tmp_path) -> None:
    run_paths = create_run_paths(tmp_path / "runs")
    paths = create_splat_paths(run_paths)
    for patch_id in ["p000", "p001"]:
        patch_splat = paths.patches / patch_id / "splat"
        patch_splat.mkdir(parents=True)
        (patch_splat / "training_status.json").write_text("{}", encoding="utf-8")

    outputs = discover_existing_splat_outputs(
        paths,
        ["splat.train"],
        train_patch_ids=["p000"],
    )

    assert len(outputs) == 1
    assert outputs[0].path == paths.patches / "p000" / "splat"

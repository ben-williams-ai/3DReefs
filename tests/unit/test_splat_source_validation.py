...
def test_validate_splat_source_uses_corrected_undistorted_images_for_gray_world(tmp_path: Path) -> None:
    ...
    assert result.paths.image_source == "corrected_undistorted"


def test_validate_splat_source_accepts_corrected_staged_names(tmp_path: Path) -> None:
    runs_dir = tmp_path / "runs"
    (runs_dir / "gray").mkdir(parents=True)
    run_paths = create_run_paths(runs_dir, run_id="gray")
    project = tmp_path / "project"
    ...
    corrected = run_paths.run_dir / "colour_restoration" / "outputs" / "undistorted" / "images"
    write_test_jpeg(corrected / "cam_1_abcd/img_000001_deadbeef.jpg")
    config = SimpleNamespace(
        colour_restoration=SimpleNamespace(mode="gray_world"),
    )

...
def test_splat_patch_generates_patch_dataset(tmp_path: Path, fake_tool_factory) -> None:
    ...
    assert metadata["selector"]["name"] == "camera_selection_v2"
    assert metadata["selector"]["selected_internal_count"] == metadata["selected_internal_count"]
    assert metadata["selector"]["selected_external_count"] == metadata["selected_external_count"]
